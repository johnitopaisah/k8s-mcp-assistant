...
def __parse_bool(value: str | None, default: bool = False) -> bool:
    if value is None:
        return default
...
class Settings:
    ...
    def load(cls) -> Settings:
        default_tail_lines = os.getenv("DEFAULT_LOG_TAIL_LINES", "100")
        max_tail_lines = os.getenv("MAX_LOG_TAIL_LINES", "1000")
        ...
        if os.getenv("KUBECONFIG") and not os.getenv("KUBECONFIG_PATH"):
            raise ValueError("KUBECONFIG_PATH must be set when KUBECONFIG is set")
        return cls(
            allowed_namespaces=_parse_csv_env(os.getenv("ALLOWED_NAMESPACES")),
            read_only=__parse_bool(os.getenv("READ_ONLY"), default=True),
            ...
            default_log_tail_lines=int(default_tail_lines),
            max_log_tail_lines=int(max_tail_lines),
        )
